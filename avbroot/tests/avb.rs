...
fn get_test_key_ml_dsa() -> SigningPrivateKey {
    let data = include_str!(concat!(
        env!("CARGO_WORKSPACE_DIR"),
        "/e2e/keys/avb_ml_dsa/TEST_KEY_DO_NOT_USE.key",
    ));
    let passphrase = include_str!(concat!(
        env!("CARGO_WORKSPACE_DIR"),
        "/e2e/keys/avb_ml_dsa/TEST_KEY_DO_NOT_USE.pass",
    ));
...
fn round_trip_root_image() {
    for (key, algorithm_type, digest) in [
        (
            get_test_key_rsa(),
        ...
        (
            ...
            [
                0x2b, 0x51, 0x16, 0xfe, 0x60, 0xe4, 0xf7, 0x1e, 0x73, 0x72, 0xf3, 0xf6, 0x21, 0x80,
                0x6c, 0x94, 0x85, 0x73, 0xac, 0xed, 0x3e, 0x41, 0x3e, 0x1c, 0x56, 0x41, 0xb0, 0x48,
                0xca, 0xed, 0x04, 0xd7, 0x63, 0xd6, 0xf7, 0xb0, 0x0d, 0x7b, 0xfd, 0xee, 0x81, 0xd1,
                0x32, 0xc0, 0xee, 0xe7, 0x96, 0x13, 0xd7, 0x3c, 0x78, 0x5b, 0x92, 0x61, 0x32, 0x18,
                0xb8, 0xd0, 0x97, 0x30, 0x16, 0x75, 0xb6, 0x10,
            ],
        ),
    ] {
...
fn round_trip_appended_hash_image() {
    for (key, algorithm_type, digest) in [
        (
            ...
            AlgorithmType::Sha256Rsa4096,
            [
                0x53, 0x71, 0x85, 0x5f, 0x2d, 0x17, 0xf9, 0x34, 0x1f, 0xeb, 0x19, 0xa3, 0xef, 0xa4,
                0xf8, 0xe0, 0x28, 0x31, 0xba, 0x84, 0xd8, 0xda, 0xe2, 0xef, 0xdb, 0x51, 0x38, 0x50,
                0x90, 0x08, 0xba, 0x59, 0xe9, 0x28, 0xf5, 0xdc, 0x15, 0x39, 0x78, 0x38, 0x15, 0xca,
                0x13, 0x16, 0xb9, 0x73, 0x48, 0x6f, 0x4e, 0xac, 0xc5, 0xa9, 0xd4, 0xa4, 0x44, 0x85,
                0x4e, 0x8c, 0xd8, 0x2f, 0x43, 0xfc, 0xbd, 0x67,
            ],
        ),
        (
            get_test_key_ml_dsa(),
            AlgorithmType::MlDsa65,
            [
                0x68, 0x5e, 0x9c, 0xfe, 0xc0, 0x19, 0x0d, 0xac, 0x2d, 0x7f, 0x74, 0x2d, 0xa0, 0x69,
                0x24, 0xb0, 0x65, 0x4e, 0x82, 0x7d, 0xc8, 0xf0, 0xef, 0x26, 0xfa, 0x7c, 0xab, 0xc6,
                0x51, 0xb1, 0x6c, 0xef, 0xc6, 0x0b, 0x57, 0x40, 0xdd, 0x60, 0xa4, 0x5d, 0xc7, 0xfe,
                0xd1, 0x03, 0x07, 0x7b, 0xad, 0x37, 0xab, 0xcc, 0xae, 0xfe, 0xe5, 0xd5, 0xb1, 0x42,
                0x49, 0x6f, 0x1b, 0xb6, 0xb5, 0xd3, 0x61, 0x50,
            ],
        ),
    ] {
        let image_size = 16384;
...
fn round_trip_appended_hash_tree_image_fixed_size() {
    for (key, algorithm_type, digest) in [
        ...
        (
            ...
            AlgorithmType::MlDsa65,
            [
                0xb8, 0x65, 0x29, 0x28, 0xb2, 0x44, 0xa9, 0x41, 0xdb, 0x80, 0xd8, 0x35, 0xb7, 0x7d,
                0x4b, 0x66, 0x2b, 0x50, 0x36, 0xeb, 0xa6, 0x83, 0x74, 0xda, 0xf1, 0x02, 0x24, 0xf3,
                0x74, 0x9f, 0xb0, 0x30, 0x18, 0x06, 0x64, 0x29, 0xa6, 0x1f, 0x18, 0xf1, 0xb5, 0xca,
                0x0f, 0x19, 0x8e, 0x9b, 0x31, 0x78, 0x3d, 0x01, 0xc8, 0x7b, 0xf6, 0xa8, 0x8b, 0x45,
                0x42, 0x8a, 0x8f, 0x9d, 0x66, 0x09, 0xa8, 0xa1,
            ],
        ),
    ] {
...
fn round_trip_appended_hash_tree_image_minimum_size() {
    for (key, algorithm_type, digest, minimum_size) in [
        (
            get_test_key_rsa(),
            AlgorithmType::Sha256Rsa4096,
            [
        ...
        assert_eq!(new_header, header);
        assert_eq!(new_footer, footer);
        assert_eq!(new_image_size, minimum_size);
    }
}

#[test]
fn corrupted_root_image() {
    for (key, algorithm_type) in [
        (get_test_key_rsa(), AlgorithmType::Sha256Rsa4096),
        (get_test_key_ml_dsa(), AlgorithmType::MlDsa65),
    ] {
        let mut header = Header {
            required_libavb_version_major: 1,
            required_libavb_version_minor: 0,
        ...
        assert_matches!(header.verify(), Err(avb::Error::HeaderVerify(_)));
        header.signature[0] = !header.signature[0];

        if !header.hash.is_empty() {
            header.hash[0] = !header.hash[0];
            assert_matches!(header.verify(), Err(avb::Error::InvalidHeaderDigest { .. }));
            header.hash[0] = !header.hash[0];
        }
    }
}
