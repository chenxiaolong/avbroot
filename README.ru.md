...
* Устройство должно поддерживать установку пользовательского публичного ключа для подтверждения статуса доверия загрузчика. Обычно это производится с помощью команды `fastboot flash avb_custom_key`. Поддержка гарантируется только на смартфонах Google Pixel с разблокируемым загрузчиком, начиная с Pixel 2.

  * ПРИМЕЧАНИЕ: Девайсы от OnePlus больше не поддерживаются в связи с обновлениями загрузчика, в которых вырезан функционал для работы пользовательского ключа. Некоторые устройства просто не загружаются ([#186,](https://github.com/chenxiaolong/avbroot/issues/186) [#195,](https://github.com/chenxiaolong/avbroot/discussions/195) [#212](https://github.com/chenxiaolong/avbroot/issues/212)), в то время как другие и вовсе могут окирпичиться ([#290.](https://github.com/chenxiaolong/avbroot/issues/290))
