...
1. Generate the AVB and OTA signing keys.
    ...
    avbroot also supports post-quantum ML-DSA keys (`-t mldsa65`) for the AVB signing key, but there are no devices with bootloaders that support these keys yet.
...
The commands above are provided for convenience. avbroot is compatible with any standard PKCS#8-encoded RSA/ML-DSA private key and PEM-encoded X509 certificate, like those generated by openssl.
